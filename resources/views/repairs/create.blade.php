<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">新建维修单</h2>
            <a href="{{ route('repairs.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; 返回列表</a>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('repairs.store') }}" id="repairForm" enctype="multipart/form-data"
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="repair_date" value="{{ old('repair_date', date('Y-m-d')) }}" required
                    ...
                    @error('repair_date')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <select name="fault_category" class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach(\App\Models\Repair::FAULT_CATEGORIES as $cat)
                            <option value="{{ $cat }}" {{ old('fault_category') === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="repair_method" class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach(\App\Models\Repair::REPAIR_METHODS as $method)
                            <option value="{{ $method }}" {{ old('repair_method') === $method ? 'selected' : '' }}>{{ $method }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">维修单位/人员</label>
                    <input type="text" name="vendor" value="{{ old('vendor') }}" placeholder="填写维修单位或联系人"
                           class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div>
                ...
                <div class="sm:col-span-3">
                    ...
                    <textarea name="fault_description" rows="3" placeholder="请详细描述故障现象和原因..."
                              class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('fault_description') }}</textarea>
                    @error('fault_description')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="sm:col-span-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">备注</label>
                    <input type="text" name="remarks" value="{{ old('remarks') }}" placeholder="选填"
                           class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-4">
            ...
            <div x-show="showPicker" x-transition class="border border-gray-200 rounded-lg p-4">
                ...
                <div class="overflow-x-auto max-h-64 overflow-y-auto">
                    ...
                    <p x-show="searchResults.length === 0 && searchQ.length === 0" class="text-center text-gray-400 py-4">输入关键词搜索资产</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-4">
            ...
            <p class="mt-1 text-xs text-gray-400">支持 jpg、png、pdf、doc、xls、zip 等格式，单个文件不超过 10MB</p>
        </div>
    ...
    function repairForm() {
        ...
        return {
            ...
            init() {
                ...
                this.selectedAssets = [@json($asset)];
