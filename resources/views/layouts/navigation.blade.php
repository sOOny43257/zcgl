...
<aside x-data="{ collapsed: false }" class="fixed inset-y-0 left-0 z-50 flex flex-col sidebar-glass text-white transition-all duration-300"
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
        ...
        <a href="{{ route('assets.importLogs') }}"
           class="flex items-center space-x-3 px-3 py-2.5 rounded-2xl transition-all duration-200 {{ request()->routeIs('assets.importLogs') ? 'bg-white/15 text-white shadow-lg shadow-black/10' : 'text-gray-400 hover:text-white hover:bg-white/8' }}">
            ...
            <span x-show="!collapsed" class="text-sm font-medium whitespace-nowrap">导入操作日志</span>
        </a>
        @endif
        ...
        <div x-data="{ processVoidOpen: {{ $processVoidActive ? 'true' : 'false' }} }">
            <a href="#"
               ...
               class="flex items-center space-x-3 px-3 py-2.5 rounded-2xl transition-all duration-200 cursor-pointer {{ $processVoidActive ? 'text-white' : 'text-gray-400 hover:text-white hover:bg-white/8' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                <span x-show="!collapsed" class="text-sm font-medium whitespace-nowrap flex-1">单据汇总</span>
                <svg x-show="!collapsed" class="h-3 w-3 shrink-0 transition-transform opacity-50" :class="processVoidOpen ? 'rotate-90' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            </a>
