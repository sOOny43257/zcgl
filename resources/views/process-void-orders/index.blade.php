<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800">流程单汇总</h2>
                <p class="text-sm text-gray-500 mt-0.5">上传 Word 模板提取表单信息，支持保存草稿与提交作废流程</p>
            </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100/50 overflow-hidden">
        <div class="p-4 border-b bg-gray-50/30">
            <form method="GET" action="{{ route('process-void-orders.index') }}" class="flex items-center gap-3 flex-wrap">
                <div class="flex-1 min-w-[220px] relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="搜索单号、科所、企业名称、税号、流程名称..." class="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500">
                ...
                <a href="{{ route('process-void-orders.index') }}" class="px-4 py-2 border border-red-200 text-red-600 text-sm rounded-xl hover:bg-red-50">重置</a>
                @endif
            </form>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-100">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">科所</th>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">企业名称</th>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">税号</th>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">流程名称</th>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">状态</th>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">是否提交纸质单据</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50">
                        ...
                        <td class="px-3 py-2.5 text-sm text-gray-600">{{ Str::limit($order->process_name, 24) ?: '-' }}</td>
                        ...
                        <td class="px-3 py-2.5 text-sm space-x-2 whitespace-nowrap">
                            ...
                            <a href="{{ route('process-void-orders.download', $order) }}" class="text-blue-600 hover:text-blue-800 text-xs font-medium">下载原件</a>
                            @endif
                            @if($order->isDraft())
                            <a href="{{ route('process-void-orders.edit', $order) }}" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">编辑</a>
                            <form method="POST" action="{{ route('process-void-orders.destroy', $order) }}" class="inline" onsubmit="return confirm('确认删除此草稿？')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="9" class="px-4 py-12 text-center text-gray-500">暂无流程单记录</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
