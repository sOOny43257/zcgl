<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">编辑报废单 <span class="text-sm text-gray-400 font-mono">（草稿）</span></h2>
            <a href="{{ route('disposals.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; 返回列表</a>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('disposals.update', $disposal) }}" id="disposalForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="disposal_date" value="{{ old('disposal_date', $disposal->disposal_date?->format('Y-m-d')) }}" required class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">处置方式 <span class="text-red-500">*</span></label>
                    <select name="disposal_method" required class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm">
                        @foreach(['报废处置','捐赠','回收'] as $m)
                ...
                <div>
                    ...
                    <input type="text" name="approver" value="{{ old('approver', $disposal->approver) }}" class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm">
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">报废原因</label>
                    <input type="text" name="reason" value="{{ old('reason', $disposal->reason) }}" class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">备注</label>
                    <input type="text" name="remarks" value="{{ old('remarks', $disposal->remarks) }}" class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm">
                </div>
            </div>
        </div>
                        ...
                        <tbody>
                            <template x-for="a in searchResults" :key="a.id">
                                <tr class="border-t border-gray-100 hover:bg-blue-50 cursor-pointer" @click="toggleAsset(a)">
                                    <td class="px-3 py-2"><input type="checkbox" :checked="isSelected(a.id)" class="rounded border-gray-300" @click.stop="toggleAsset(a)"></td>
        ...
        <div class="flex justify-between">
            <form method="POST" action="{{ route('disposals.update', $disposal) }}" onsubmit="return confirm('确定删除此草稿？')">
                @csrf @method('PUT')
    ...
    function disposalAssets() {
        return {
            ...
            isSelected(id) { return this.selectedAssets.some(a => a.id === id); },
            toggleAsset(asset) {
                const idx = this.selectedAssets.findIndex(a => a.id === asset.id);
                if (idx >= 0) this.selectedAssets.splice(idx, 1);
                else this.selectedAssets.push(asset);
            },
            removeAsset(idx) { this.selectedAssets.splice(idx, 1); }
        };
    }
    function submitDraft() { document.getElementById('formAction').value = 'save'; document.getElementById('disposalForm').submit(); }
    function submitFinal() { document.getElementById('formAction').value = 'submit'; document.getElementById('disposalForm').submit(); }
    </script>
</x-app-layout>
