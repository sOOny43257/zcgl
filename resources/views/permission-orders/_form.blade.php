    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            <div>
                ...
                <input type="text" name="voided_by" form="permissionForm" value="{{ old('voided_by') }}" class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-red-500" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">作废时间 <span class="text-red-500">*</span></label>
                <input type="datetime-local" name="voided_at" form="permissionForm" value="{{ old('voided_at', date('Y-m-d\TH:i')) }}" class="w-full border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-red-500" required>
            </div>
            <label class="flex items-center gap-2 text-sm text-gray-600">
                <input type="checkbox" name="paper_submitted" form="permissionForm" value="1" class="rounded border-gray-300 text-red-600 focus:ring-red-500" {{ old('paper_submitted') ? 'checked' : '' }}>
                已提交纸质单据
            </label>
        </div>
        <div class="px-6 py-3 border-t bg-gray-50 flex justify-end gap-2">
            ...
            <button type="button" onclick="document.getElementById('permissionAction').value='submit';document.getElementById('permissionForm').submit();" class="px-4 py-2 text-sm text-white bg-red-600 rounded-lg hover:bg-red-700">确认提交</button>
        </div>
    </div>
</div>
