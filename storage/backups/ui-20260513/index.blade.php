                            ...
                            <tr class="hover:bg-gray-50/50 text-sm">
                                ...
                                <template x-for="col in columns" :key="col.field">
                                    <td x-show="col.visible" class="px-3 py-2">
                                        <template x-if="col.field === 'name'">
                                            <a :href="'/zcgl/public/assets/' + a.id" class="text-blue-600 hover:underline" x-text="a[col.field] || '-'"></a>
                                        </template>
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex gap-2 text-xs whitespace-nowrap">
                                        <a :href="'/zcgl/public/assets/' + a.id" class="text-blue-600 hover:underline">查看</a>
                                        @if(auth()->user()->isAdmin())
                                        <a :href="'/zcgl/public/assets/' + a.id + '/edit'" class="text-indigo-600 hover:underline">编辑</a>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('assetIndex', () => ({
        ...
        async loadFilterOptions() {
            try {
                const res = await fetch('/zcgl/public/assets/search?q=');
                if (!res.ok) return;
                const data = await res.json();
                const fieldMap = { departments: ['department','department_name'], statuses: ['status','status_name'], categories: ['category','category_name'], brands: ['brand','brand'], rooms: ['room','room'], users: ['user','user'] };
        ...
        async load() {
            ...
            try {
                const res = await fetch('/zcgl/public/assets/json?' + params.toString());
                if (res.ok) {
        ...
        async deleteAsset(id) {
            ...
            try {
                await fetch('/zcgl/public/assets/' + id, {
                    method: 'DELETE',
